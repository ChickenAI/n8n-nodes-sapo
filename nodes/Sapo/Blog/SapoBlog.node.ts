		...
		properties: [
			...
			},
			{
				...
				options: [
					...
					{ name: 'Tạo Bình Luận', value: 'createComment' },
					{ name: 'Xóa', value: 'delete' },
					{ name: 'Xóa Bình Luận', value: 'deleteComment' },
					{ name: 'Lấy Chi Tiết', value: 'get' },
					{ name: 'Lấy Bình Luận', value: 'getComment' },
					{ name: 'Lấy Danh Sách Bình Luận', value: 'getComments' },
					{ name: 'Lấy Danh Sách', value: 'getMany' },
					{ name: 'Kiểm Tra Spam', value: 'spamCheck' },
					{ name: 'Cập Nhật', value: 'update' },
					{ name: 'Cập Nhật Bình Luận', value: 'updateComment' },
				],
				default: 'getMany',
			},
			{
			...
			if (resource === 'blog') {
				...
				if (operation === 'create') {
					...
					if (returnAll) {
						...
						const limit = this.getNodeParameter('limit', 0) as number;
						const response = await blogs.listComments(blogId, articleId, {
							...filters,
							limit,
						} as any);
						responseData = response as unknown as SapoResponse;
